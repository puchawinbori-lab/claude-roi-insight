...
interface HoursPerTicketChartProps {
  timeSeriesData: any[];
  claudeAdoptionDate: string;
}

const HoursPerTicketChart = ({ timeSeriesData, claudeAdoptionDate }: HoursPerTicketChartProps) => {
  // Create data for all weeks from the dataset
  const chartData = timeSeriesData
    .filter(item => item.Week_Start && item.Avg_Hours_Per_Task != null) // Filter out invalid data
    .map((item) => {
      ...
      return {
        week: formattedDate,
        avgHours: item.Avg_Hours_Per_Task,
  ...
  return (
    <div className="bg-card rounded-xl p-6 border shadow-sm animate-fade-in">
      <div className="mb-4">
        <h3 className="text-lg font-semibold">Average Hours per Ticket Over Time</h3>
        <p className="text-sm text-muted-foreground">Weekly trends with Claude Code adoption marker</p>
      </div>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <YAxis
            stroke="hsl(var(--muted-foreground))"
            fontSize={12}
            label={{ value: "Avg Hours/Ticket", angle: -90, position: "insideLeft" }}
          />
          <Tooltip
            contentStyle={{
            ...
            formatter={(value: number) => [value.toFixed(1), "Avg Hours/Ticket"]}
          />
...
export default HoursPerTicketChart;
